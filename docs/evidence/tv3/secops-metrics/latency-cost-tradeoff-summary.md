...
|---|---|---|---|---|---|
| Kong Gateway routing over local HTTP lab path | TCP accept, route lookup, upstream proxying, headers | p50/p95 from health requests; latest full k6 evidence exists, smoke output is to be filled by running script | Run a small curl-based smoke against health endpoints through `BASE_URL` | `BASE_URL=http://localhost:8000 REQUESTS=5 bash scripts/metrics/latency-overhead-smoke.sh` | Low operational cost in lab; adds a central enforcement point and modest latency; acceptable for SME APIs when p95 stays within SLA |
...
|---|---|---|
| `docs/evidence/tv3/metrics/k6-gateway-waf-latency.md` | measured full k6 lab run | Contains existing p50/p95 and threshold evidence from 2026-06-17 |
...
| `docs/evidence/tv3/metrics/kms-vault-call-overhead.md` | measured startup secret retrieval overhead | Treats Vault/KMS as startup/rotation overhead, not per-request overhead |
