...
if bash "${PROJECT_ROOT}/demo/webhook/send-valid-webhook.sh" > /tmp/edge-webhook-valid.log 2>&1; then
  pass "valid webhook accepted"
else
  fail "valid webhook rejected (see /tmp/edge-webhook-valid.log)"
fi
...
if bash "${PROJECT_ROOT}/demo/webhook/send-invalid-signature.sh" > /tmp/edge-webhook-invalid.log 2>&1; then
  ...
  if grep -q "HTTP_STATUS:401" /tmp/edge-webhook-invalid.log 2>/dev/null; then
    pass "invalid webhook rejected (401)"
  else
    fail "invalid webhook unexpectedly accepted (see /tmp/edge-webhook-invalid.log)"
  fi
fi
...
if bash "${PROJECT_ROOT}/demo/webhook/send-replay-webhook.sh" > /tmp/edge-webhook-replay.log 2>&1; then
  ...
  if grep -q "HTTP_STATUS:200" /tmp/edge-webhook-replay.log 2>/dev/null && \
     grep -q "HTTP_STATUS:403" /tmp/edge-webhook-replay.log 2>/dev/null; then
    pass "replay detected: attempt 1 accepted, attempt 2 rejected"
  elif grep -q "HTTP_STATUS:403" /tmp/edge-webhook-replay.log 2>/dev/null; then
    pass "replay webhook rejected on retry (403)"
  else
    fail "replay webhook unexpected behavior (see /tmp/edge-webhook-replay.log)"
  fi
fi
